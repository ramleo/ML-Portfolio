    ...
    <div className="flex flex-col gap-1.5">
      <div className="relative mb-0.5">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="w-full text-[11px] bg-[var(--bg-glass)] border border-[var(--border)] rounded-lg pl-6 pr-5 py-1.5 text-[var(--text)] placeholder:text-[var(--text3)] outline-none focus:border-indigo-500/40 transition-colors"/>
        {search && (
          <button onClick={() => setSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--text3)] hover:text-[var(--text)] transition-colors">
      ...
      {!filtered.length && <p className="text-[11px] text-[var(--text3)] text-center py-2">No match</p>}
      ...
      {filtered.map(([tn, t]) => {
        ...
        return (
          <div key={tn} className="rounded-lg border border-[var(--border)] overflow-hidden bg-white/[0.02]">
            ...
            {isOpen && (
              <div className="border-t border-[var(--border)] px-2 py-1.5 flex flex-col gap-0.5">
                {(() => {
                  ...
                  return t.columns.map(c => {
                    ...
                    return (
                      <div key={c.name} className="flex items-center gap-1.5 py-[2px]">
                        ...
                        <span className={`text-[11px] font-mono truncate ${hi ? "text-amber-300" : c.pk ? "text-indigo-300/90" : fkTarget ? "text-violet-300" : "text-[var(--text2)]"}`}>
                          {c.name}
                        </span>
