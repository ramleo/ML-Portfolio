...
export default function SchemaPanel({ schema, accent }: Props) {
  ...
  if (!schema) return (
    <div className="flex flex-col items-center gap-2 py-6 px-2">
      ...
      <p className="text-[11px] text-gray-600 text-center">Load a database<br/>to explore schema</p>
    </div>
  );
    ...
    <div className="flex flex-col gap-1.5">
      <div className="relative mb-0.5">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="w-full text-[11px] bg-black/40 border border-white/8 rounded-lg pl-6 pr-5 py-1.5 text-gray-300 placeholder-gray-600 outline-none focus:border-indigo-500/40 transition-colors"/>
        {search && (
      ...
      {!filtered.length && <p className="text-[11px] text-gray-600 text-center py-2">No match</p>}
        ...
        return (
          <div key={tn} className="rounded-lg border border-white/5 overflow-hidden bg-white/[0.02]">
            <button onClick={() => toggle(tn)}
              ...
              <span className="text-[11px] font-mono font-medium text-gray-200 flex-1 truncate">{tn}</span>
              <span className="text-[11px] text-gray-600 tabular-nums shrink-0">{t.row_count.toLocaleString()}</span>
                  ...
                  return t.columns.map(c => {
                    ...
                    return (
                      <div key={c.name} className="flex items-center gap-1.5 py-[2px]">
                        {c.pk
                          ? <span className="text-[11px] px-1 py-px rounded font-bold shrink-0" style={{ background: `${accent}22`, color: accent }}>PK</span>
                          : fkTarget
                            ? <span title={fkTarget} className="text-[11px] px-1 py-px rounded font-bold shrink-0 cursor-help" style={{ background: "#6d28d940", color: "#c4b5fd" }}>FK</span>
                            : <span className="text-[11px] px-1 py-px rounded font-medium shrink-0" style={{ background: tc.bg, color: tc.text }}>{tc.label}</span>
                        }
                        <span className={`text-[11px] font-mono truncate ${hi ? "text-amber-300" : c.pk ? "text-indigo-300/90" : fkTarget ? "text-violet-300" : "text-gray-400"}`}>
                          {c.name}
                        </span>
                        {fkTarget && <span className="text-[11px] text-violet-400/70 shrink-0 font-sans">{fkTarget.split(".")[0]}</span>}
                      </div>
                    );
                  });
                })()}
