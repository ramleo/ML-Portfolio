...
import { PROMPT_INJECTION_GUIDE } from "./userGuide";
...
export default function PromptInjectionUserGuideModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  useEffect(() => {
    if (!open) return;
      ...
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b shrink-0"
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.12em]" style={{ color: ACCENT }}>
            User Guide — LLM Prompt Injection Detection Playground
          </span>
        ...
        <div className="overflow-y-auto px-6 py-4">
          <ReactMarkdown components={MD}>{PROMPT_INJECTION_GUIDE}</ReactMarkdown>
        </div>
      </div>
    </div>
  );
