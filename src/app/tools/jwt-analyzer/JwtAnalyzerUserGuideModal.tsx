...
import { JWT_GUIDE } from "./userGuide";
...
export default function JwtAnalyzerUserGuideModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  useEffect(() => {
    if (!open) return;
      ...
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b shrink-0"
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.12em]" style={{ color: ACCENT }}>
            User Guide — JWT / Token Security Analyzer
          </span>
        ...
        <div className="overflow-y-auto px-6 py-4">
          <ReactMarkdown components={MD}>{JWT_GUIDE}</ReactMarkdown>
        </div>
      </div>
    </div>
  );
