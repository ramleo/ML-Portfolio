...
import { FACE_CLOAK_GUIDE } from "./userGuide";
...
export default function FaceCloakUserGuideModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  useEffect(() => {
    if (!open) return;
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl overflow-hidden"
        ...
        <div className="overflow-y-auto px-6 py-4">
          <ReactMarkdown components={MD}>{FACE_CLOAK_GUIDE}</ReactMarkdown>
        </div>
      </div>
    </div>
  );
