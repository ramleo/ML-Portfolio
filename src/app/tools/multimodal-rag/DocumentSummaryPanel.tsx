...
import type { Bbox, DetectedObject, IngestState, TranscriptSegment } from "./_types";
...
type Props = {
  ...
  onSelectChunk: (chunkType: string | null | undefined, page: number | null | undefined, text: string, bbox?: Bbox | null, objects?: DetectedObject[] | null, timestampS?: number | null, piiTypes?: string | null) => void;
      ...
      {d.notableChunks.length === 0 ? (
        ...
        d.notableChunks.map((c, i) => (
          <RagSourceCard key={i} source={d.source} text={c.text} score={1} accent={accent}
            ...
            blurry={c.blurry}
            onSelect={() => onSelectChunk(c.chunkType, c.page, c.text, c.bbox, c.objects, c.timestampS, c.piiTypes)}
          />
        ))
      )}
    </div>
