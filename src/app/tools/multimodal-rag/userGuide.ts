...
automatically retries with another provider behind the scenes — you'll just
get your answer, possibly a few seconds slower, instead of an error. This
only happens before an answer has started; if a provider fails partway
through writing a response, that's shown as an error rather than silently
switched, since splicing two providers' text together would produce a
garbled answer.
