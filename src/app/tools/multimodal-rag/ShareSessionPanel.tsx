...
export default function ShareSessionPanel({ sessionId, accent }: { sessionId: string; accent: string }) {
  ...
  if (share) {
    ...
    return (
      <div className="flex items-center gap-1.5 flex-wrap text-[12px]" style={{ color: "var(--text2)" }}>
        ...
        <span style={{ color: "var(--text3)" }}>Expires in ~{expiresIn}h</span>
    ...
    <div className="relative">
      ...
      {warnOpen && (
        <div className="absolute z-10 top-full mt-1.5 left-0 w-72 p-3 rounded-lg text-[12px] leading-relaxed"
          style={{ background: "var(--bg-card)", border: "1px solid rgba(245,158,11,0.3)", color: "var(--text2)" }}>
          <p className="mb-2">
