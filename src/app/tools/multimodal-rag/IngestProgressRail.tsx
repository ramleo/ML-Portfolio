          ...
          try {
            ...
            if (evt.done) {
              const result: Extract<IngestState, { kind: "done" }> = {
                ...
                embeddingMode,
                notableChunks: (evt.notable_chunks ?? []).map((c: { chunk_type: string | null; page: number | null; text: string; timestamp_s?: number | null; bbox?: [number, number, number, number] | null; objects?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; signatures?: { label: string; confidence: number; bbox: [number, number, number, number]; mask?: [number, number][] | null }[] | null; tampering?: { label: string; confidence: number; bbox: [number, number, number, number]; mask?: [number, number][] | null }[] | null; duplicates?: { source: string; page: number; similarity: number }[] | null; steganography?: { detected: boolean; confidence: number; payload_type?: string | null } | null; moire?: { detected: boolean; confidence: number } | null; number_mismatch?: boolean | null; pii_types?: string | null; blurry?: boolean | null; entities?: { type: string; value: string }[] | null; person_count?: number | null }) => ({
                  chunkType: c.chunk_type, page: c.page, text: c.text, timestampS: c.timestamp_s ?? null, bbox: c.bbox ?? null, objects: c.objects ?? null, signatures: c.signatures ?? null, tampering: c.tampering ?? null, duplicates: c.duplicates ?? null, steganography: c.steganography ? { detected: c.steganography.detected, confidence: c.steganography.confidence, payloadType: c.steganography.payload_type ?? null } : null, moire: c.moire ?? null, numberMismatch: !!c.number_mismatch, piiTypes: c.pii_types ?? null, blurry: !!c.blurry, entities: c.entities ?? null, personCount: c.person_count ?? null,
                })),
