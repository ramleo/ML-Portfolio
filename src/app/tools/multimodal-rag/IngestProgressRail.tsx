          ...
          try {
            ...
            if (evt.done) {
              const result: Extract<IngestState, { kind: "done" }> = {
                ...
                embeddingMode,
                notableChunks: (evt.notable_chunks ?? []).map((c: { chunk_type: string | null; page: number | null; text: string; timestamp_s?: number | null; bbox?: [number, number, number, number] | null; objects?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; signatures?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; tampering?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; duplicates?: { source: string; page: number; similarity: number }[] | null; number_mismatch?: boolean | null; pii_types?: string | null; blurry?: boolean | null; entities?: { type: string; value: string }[] | null }) => ({
