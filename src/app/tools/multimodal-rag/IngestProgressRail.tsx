          ...
          try {
            ...
            if (evt.done) {
              const result: Extract<IngestState, { kind: "done" }> = {
                ...
                embeddingMode,
                notableChunks: (evt.notable_chunks ?? []).map((c: { chunk_type: string | null; page: number | null; text: string; timestamp_s?: number | null; bbox?: [number, number, number, number] | null; objects?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; signatures?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; tampering?: { label: string; confidence: number; bbox: [number, number, number, number] }[] | null; number_mismatch?: boolean | null; pii_types?: string | null; blurry?: boolean | null; entities?: { type: string; value: string }[] | null }) => ({
                  chunkType: c.chunk_type, page: c.page, text: c.text, timestampS: c.timestamp_s ?? null, bbox: c.bbox ?? null, objects: c.objects ?? null, signatures: c.signatures ?? null, tampering: c.tampering ?? null, numberMismatch: !!c.number_mismatch, piiTypes: c.pii_types ?? null, blurry: !!c.blurry, entities: c.entities ?? null,
                })),
