          ...
          try {
            ...
            if (evt.done) {
              const result: Extract<IngestState, { kind: "done" }> = {
                ...
                embeddingMode,
                notableChunks: (evt.notable_chunks ?? []).map((c: { chunk_type: string | null; page: number | null; text: string; number_mismatch?: boolean | null; pii_types?: string | null; blurry?: boolean | null }) => ({
                  chunkType: c.chunk_type, page: c.page, text: c.text, numberMismatch: !!c.number_mismatch, piiTypes: c.pii_types ?? null, blurry: !!c.blurry,
                })),
