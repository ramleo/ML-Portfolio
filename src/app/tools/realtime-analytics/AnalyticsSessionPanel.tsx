    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative z-10 flex flex-col h-full overflow-hidden"
        ...
        <div className="flex items-center gap-2 px-4 py-3 shrink-0"
          ...
          <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">Session</p>
          <code className="text-[9px] text-emerald-400/70 font-mono ml-1">{selectedSid.slice(0, 12)}…</code>
          <button onClick={onClose}
            className="ml-auto text-gray-600 hover:text-gray-400 text-[11px] transition-colors">
            ✕
          </button>
        </div>
              ...
              return (
                <div key={tool} className="mb-2.5">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[9px] text-gray-400 font-mono truncate max-w-[160px]">{tool}</span>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-[8px] tabular-nums" style={{ color: "#64748b" }}>{pct}%</span>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          <p className="text-[9px] font-semibold text-gray-600 uppercase tracking-widest mb-3">Event Path</p>
          ...
          {!sessLoading && !sessError && sessionEvs.length === 0 && (
            <p className="text-xs text-gray-600">No events found for this session.</p>
          )}
            ...
            return (
              <div key={ev.id} className="flex gap-3">
                ...
                <div className="flex flex-col items-center shrink-0">
                  ...
                  {!isLast && <div className="w-px flex-1 mt-1" style={{ background: "rgba(255,255,255,0.06)", minHeight: "20px" }}/>}
                </div>
                {/* Event content */}
                <div className="pb-3 min-w-0">
                  ...
                  <p className="text-[9px] font-mono truncate" style={{ color: "#64748b" }}>{label}</p>
                  <p className="text-[8px] mt-0.5" style={{ color: "#1f2937" }}>
