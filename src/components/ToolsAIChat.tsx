    ...
    <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 9999, fontFamily: "inherit", pointerEvents: "none" }}>
      {open && (
        <div style={{
          ...
          boxShadow: `var(--shadow), 0 0 0 1px ${accentColor}18`,
          ...
          <div style={{
            ...
            background: `linear-gradient(135deg, var(--bg) 0%, rgba(${accentColor === "#38bdf8" ? "56,189,248" : accentColor === "#f59e0b" ? "245,158,11" : "52,211,153"},0.08) 100%)`,
          }}>
            {confirmClear ? (
              <>
                ...
                <button onClick={() => setConfirmClear(false)}
                  style={{ background: "transparent", border: "1px solid var(--border2)", borderRadius: 6, color: "var(--text2)", cursor: "pointer", padding: "3px 12px", fontSize: "0.65rem", fontWeight: 700 }}>
                  No
                </button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => setDeepSearch(d => !d)}
                  ...
                  style={{ background: deepSearch ? `${accentColor}22` : "transparent", border: `1px solid ${deepSearch ? accentColor + "55" : "var(--border)"}`, borderRadius: 6, color: deepSearch ? accentColor : "var(--text3)", cursor: "pointer", padding: "3px 6px", display: "flex", alignItems: "center", gap: "3px", fontSize: "0.58rem", fontWeight: 600 }}>
                ...
                <button onClick={() => setForceWeb(w => !w)}
                  ...
                  style={{ background: forceWeb ? "#f59e0b22" : "transparent", border: `1px solid ${forceWeb ? "#f59e0b55" : "var(--border)"}`, borderRadius: 6, color: forceWeb ? "#f59e0b" : "var(--text3)", cursor: "pointer", padding: "3px 6px", display: "flex", alignItems: "center", gap: "3px", fontSize: "0.58rem", fontWeight: 600 }}>
                ...
                <button onClick={enableJina}
                  ...
                  style={{ background: useJina ? `${accentColor}22` : "transparent", border: `1px solid ${useJina ? accentColor + "55" : "var(--border)"}`, borderRadius: 6, color: useJina ? accentColor : "var(--text3)", cursor: "pointer", padding: "3px 6px", display: "flex", alignItems: "center", gap: "3px", fontSize: "0.58rem", fontWeight: 600 }}>
                  <SparkleIcon />{useJina ? "Jina" : "Std"}
                </button>
                </>}
                <button onClick={() => setSettings(s => !s)}
                  style={{ background: settings ? `${accentColor}22` : "transparent", border: `1px solid ${settings ? accentColor + "55" : "var(--border)"}`, borderRadius: 6, color: settings ? accentColor : "var(--text3)", cursor: "pointer", padding: "3px 6px", display: "flex", alignItems: "center" }}>
                  <GearIcon />
                </button>
                {messages.length > 0 && (
                  <button onClick={() => setConfirmClear(true)} title="Clear conversation"
                    style={{ background: "transparent", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text3)", cursor: "pointer", padding: "3px 6px", display: "flex", alignItems: "center" }}>
                    <TrashIcon />
                  </button>
                )}
          ...
          <ChatMessageList
            ...
            answerSource={answerSource} confidence={confidence} groundedness={groundedness}
          ...
          <div style={{ padding: "0.6rem 0.75rem", borderTop: "1px solid var(--border)", display: "flex", gap: "0.5rem", alignItems: "flex-end" }}>
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
              ...
              rows={1}
              style={{ flex: 1, resize: "none", background: "var(--border)", border: "1px solid var(--border)", borderRadius: 9, color: "var(--text)", fontSize: "0.74rem", padding: "0.45rem 0.6rem", outline: "none", lineHeight: 1.5, maxHeight: 100, overflowY: "auto", fontFamily: "inherit" }}
            />
            <button onClick={send} disabled={!input.trim() || loading}
              style={{ background: input.trim() && !loading ? accentColor : "var(--border)", border: "none", borderRadius: 9, padding: "0.45rem 0.65rem", color: input.trim() && !loading ? "#000" : "var(--text3)", cursor: input.trim() && !loading ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s", flexShrink: 0 }}>
              <SendIcon />
            </button>
          </div>
        </div>
