...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div style={{ position: "absolute", top: 60, left: 0, right: 0, background: "var(--bg-nav)", backdropFilter: "blur(16px)", borderBottom: "1px solid var(--border)", padding: "1rem 1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
          {NAV_LINKS.map((link) => (
            <a key={link.href} href={link.href} onClick={() => setMenuOpen(false)} style={{ fontSize: "0.95rem", color: "var(--text2)", textDecoration: "none", fontWeight: 500 }}>
              {link.label}
            </a>
          ))}
        </div>
